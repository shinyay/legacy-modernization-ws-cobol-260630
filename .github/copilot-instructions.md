# Copilot Instructions — practice-bank modernization

このリポは「レガシー COBOL を **ドメイン分析(Code→Doc)し、rehost コンテナで Azure へ移行**する」（Java 直訳はしない · ADR-0027）。
**原則・場所の“正”は [`AGENTS.md`]。まずそれに従う。**
本ファイルは Copilot 向けの **運用ルール** と **skill routing** だけを足す。
...
- **本番ランタイムは rehost コンテナ（COBOL+OCESQL）。** ACA Jobs/Service で動かす。COBOL の build/freeze は**コンテナ or CI**（ADR-0013）。
- ※ 直訳禁止・`legacy/` 不変・数値（BigDecimal/COMP-3）の扱いは **`AGENTS.md` が正**。
...
| Azure へ rehost コンテナ化 | `containerizer` エージェント |
